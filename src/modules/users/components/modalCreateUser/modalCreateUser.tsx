...
export const ModalCreateUser = ({ onClose, open }: ModalCreateUserProps) => {
  const { handleSubmit, alertInfo, handleCloseAlert } = useModalCreateUser({
  ...
  return (
    <>
      <Modal
        onClose={onClose}
        open={open}
        title={translate("users.title.create")}
        titleId="modal-create-user"
      >
        <UserForm handleSubmit={handleSubmit} handleCancel={onClose} />
      </Modal>
